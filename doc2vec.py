...
def load_glove(filename):
    ...
    """
    wvinfo = {}
    with open(filename, 'r') as glove:
        for line in glove.readlines():
            contents = line.split(' ')
            wvinfo[contents[0]] = np.array(contents[1:], dtype=np.float32)
    return wvinfo
...
def words(text):
    ...
    Ignore words < 3 char long, lowercase all words and Remove English stop words
    """
    regex = re.compile('[' + re.escape(string.punctuation) + '0-9\\r\\t\\n]')
    nopunct = regex.sub(" ", text)
    words = nopunct.split(" ")
    words = [w.lower() for w in words if len(w) > 2 if w not in ENGLISH_STOP_WORDS]
    return words
...
def load_articles(articles_dirname, gloves):
    ...
    """
    article_table = []
    all_files = filelist(articles_dirname)
    for file in all_files:
        contents = get_text(file)
        text = contents.split('\n', 1)
        title = text[0].strip()
        text_minus_title = text[1].strip()
        wordvec = doc2vec(contents, gloves)
        article_table.append((file, title, text_minus_title, wordvec))
...
def distances(article, articles):
    ...
    for a in articles:
        wv_a = a[3]
        distance = np.linalg.norm(wv_target - wv_a)
...
def recommended(article, articles, n):
    ...
    """
    res = distances(article, articles)
    res.sort(key=lambda x: x[0])
    res = res[1:n+1]
    return list(dis[1] for dis in res)
